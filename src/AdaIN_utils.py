...
class RC(nn.Layer):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, pad_size=1, activated=True):
        super().__init__()
        self.pad = nn.Pad2D([(pad_size, pad_size), (pad_size, pad_size)], mode='reflect')
...
def denorm(tensor, device):
    std = paddle.to_tensor([0.229, 0.224, 0.225]).reshape([-1, 1, 1]).to(device)
    mean = paddle.to_tensor([0.485, 0.456, 0.406]).reshape([-1, 1, 1]).to(device)
    res = paddle.clip(tensor * std + mean, 0, 1)
    return res
