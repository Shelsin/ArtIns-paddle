...
def to_tensor(array):
    ...
    Returns:
      A `paddle.Tensor` with dtype `paddle.float32`.
    """
    assert isinstance(array, np.ndarray)

    return paddle.to_tensor(array, dtype='float32')


def postprocess(images, min_val=-1.0, max_val=1.0):
    """Post-processes images from `paddle.Tensor` to `numpy.ndarray`.

    Args:
        images: A `paddle.Tensor` with shape `NCHW` to process.
    ...
    """
    assert isinstance(images, paddle.Tensor)
    images = images.numpy()
